...
WHERE id=?', 'i', [$userId])->fetch_array(MYSQLI_ASSOC);
    ...
    public function getEmailList(int $userId): array
    {
        ...
        $data = $this->executeQuery('SELECT id,emailEncrypted as email FROM usersEmails WHERE userId=? and deleted=false', 'i', [$userId])->fetch_all(MYSQLI_ASSOC);
        return $data;
    }
    ...
    public function getPhonesList(int $userId): array
    {
        ...
        $data = $this->executeQuery('SELECT id,phoneEncrypted as phone FROM usersPhones WHERE userId=? and deleted=false', 'i', [$userId])->fetch_all(MYSQLI_ASSOC);
        return $data;
    }
    ...
    public function checkPhoneInDatabase(string $phoneHash): bool
    {
        ...
        $data = $this->executeQuery('SELECT COUNT(*) as count FROM usersPhones WHERE phoneHash=?', 's', [$phoneHash])->fetch_array(MYSQLI_ASSOC);
        return $data['count'] > 0;
    }
